classdef LinkModel < handle
    ...
    methods
        ...
        function b = checkLinkBusy( obj, t )
            ...
            switch obj.proto
                case 'ALOHA'
                    b = 0;
                case 'CSMA' 
                    if obj.busy == 0 % 发送，如果通道空闲
                        obj.mac.flush();
                        b = 0;
                    else                        
                        if obj.mac.retry < obj.mac.maxretry
                            if obj.mac.timeout == 0 % 计时器未运行
                                obj.mac.retry = 1;
        ...
        %定义触发事件
        function obj = timeout(obj,t) % 链路繁忙时间的下行计数器，按节点的采样率处理
            if obj.until > 0
                obj.until = obj.until - t;
                if obj.until <= 0
                    %fprintf('link timeout at %d\r\n',obj.id);
                    notify(obj,'finishedSending');   % 链接再次可用
                end
            end
        end
